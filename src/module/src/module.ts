...
export default defineNuxtModule<ModuleOptions>({
  ...
  async setup(options, nuxt) {
    ...
    nuxt.options.runtimeConfig.studio = {
      ...
      // @ts-expect-error Custom options for access control and GitHub App
    ...
    // GitHub App token endpoint for bot-attributed commits
    addServerHandler({
      route: '/api/studio/github-app-token',
      handler: runtime('./server/routes/github-app-token.get'),
    })

    // Auth middleware for allowed users check
